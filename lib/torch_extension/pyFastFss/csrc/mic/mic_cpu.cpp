...
std::tuple<torch::Tensor&, torch::Tensor&> cpu_dcf_mic_key_gen(torch::Tensor&       key,
                                                               torch::Tensor&       z,
                                                               ...
                                                               std::size_t          elementNum)
{
    ...
    if (ret != 0)
    {
        std::string msg = "FastFss_cpu_dcfMICKeyGen failed : " + std::to_string(ret);
        throw std::runtime_error(msg);
    }
    return std::tuple<torch::Tensor&, torch::Tensor&>(key, z);
}

torch::Tensor& cpu_dcf_mic_eval(torch::Tensor&       out,
                                ...
                                std::size_t          elementNum)
{
    ...
    if (ret != 0)
    {
        std::string msg = "FastFss_cpu_dcfMICEval failed : " + std::to_string(ret);
        throw std::runtime_error(msg);
    }
    return out;
}
