...
std::tuple<torch::Tensor &, torch::Tensor &, torch::Tensor &> cuda_spline_key_gen(torch::Tensor       &key,
                                                                                  torch::Tensor       &e,
                                                                                  torch::Tensor       &beta,
                                                                                  ...
                                                                                  std::size_t          elementNum)
{
    ...
    int ret = FastFss_cuda_dcfSplineKeyGen(                     //
    ...
    if (ret != 0)
    {
        std::string msg = "FastFss_cuda_dcfSplineKeyGen failed : " + std::to_string(ret);
        throw std::runtime_error(msg);
    }
    return std::tuple<torch::Tensor &, torch::Tensor &, torch::Tensor &>(key, e, beta);
}

torch::Tensor &cuda_spline_eval(torch::Tensor       &out,
                                ...
                                std::size_t          elementNum)
{
    ...
    ret = FastFss_cuda_dcfSplineEval(                           //
    ...
    if (ret != 0)
    {
        std::string msg = "FastFss_cuda_dcfSplineEval failed : " + std::to_string(ret);
        throw std::runtime_error(msg);
    }
    return out;
}
