...
def test(bit_width: int, lut_bit_width: int, dtype: torch.dtype, device: torch.device):
    ...
    points = torch.arange(0, 1 << lut_bit_width, device=device, dtype=dtype)
    st = time.time_ns()
    shared_out_e0, shared_out_t0 = pyFastFss.grotto_lut_eval_ex2(
        shared_out_e0, shared_out_t0, masked_x, key, seed0, 0, points,
        luts, bit_width, bit_width_out, element_num
    )
    torch.cuda.synchronize()
    et = time.time_ns()
    print("\tgrotto_lut_eval_ex2(0)", (et - st) / 1e6, "ms")
    st = time.time_ns()
    shared_out_e1, shared_out_t1 = pyFastFss.grotto_lut_eval_ex2(
        shared_out_e1, shared_out_t1, masked_x, key, seed1, 1, points,
        luts, bit_width, bit_width_out, element_num
    )
    torch.cuda.synchronize()
    et = time.time_ns()
    print("\tgrotto_lut_eval_ex2(1)", (et - st) / 1e6, "ms")
    ...
    print("[TEST] grotto_lut_eval_ex2:",
