...
def test(bit_width: int, dtype: torch.dtype, device: torch.device):
    shape = (12, 128)
    ...
    luts = torch.arange(0, 2 ** bit_width, dtype=dtype, device=device)

    st = time.time_ns()
    shared_out_e0, shared_out_t0 = pyFastFss.grotto_lut_eval(
        shared_out_e0, shared_out_t0, masked_x, key, seed0, 0, luts, bit_width, bit_width_out, element_num
    )
    torch.cuda.synchronize()
    et = time.time_ns()
    print("\tgrotto_lut_eval(0)", (et - st)/1e6, "ms")
    st = time.time_ns()
    shared_out_e1, shared_out_t1 = pyFastFss.grotto_lut_eval(
        shared_out_e1, shared_out_t1, masked_x, key, seed1, 1, luts, bit_width, bit_width_out, element_num
    )
    torch.cuda.synchronize()
    et = time.time_ns()
    print("\tgrotto_lut_eval(1)", (et - st)/1e6, "ms")
...
for i in range(len(dtype_lst)):
    test(7, dtype_lst[i], torch.device("cpu"))
for i in range(len(dtype_lst)):
    test(7, dtype_lst[i], torch.device("cuda"))
