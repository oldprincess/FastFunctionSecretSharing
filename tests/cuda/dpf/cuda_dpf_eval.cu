...
template <typename T>
class CudaDpfEvalTest : public ::testing::Test
{
};

TYPED_TEST_SUITE(CudaDpfEvalTest, EvalElementTypes);

TYPED_TEST(CudaDpfEvalTest, EvalPlacesBetaAtZeroOffset)
{
    using T                         = TypeParam;
    ...
    for (const bool useStreams : {false, true})
    {
        ...
        for (const auto &widths : eval_width_configs<T>())
        {
            for (const auto elementNum : element_counts())
            {
                ...
                ASSERT_EQ(
                    FastFss_cuda_dpfEval(dShare0.get(), elementNum * groupSize * sizeof(T), dMaskedX.get(),
                    ...
                    FAST_FSS_SUCCESS);
                ASSERT_EQ(
                    FastFss_cuda_dpfEval(dShare1.get(), elementNum * groupSize * sizeof(T), dMaskedX.get(),
