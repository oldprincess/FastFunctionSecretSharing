#ifndef SRC_IMPL_AES_H
#define SRC_IMPL_AES_H
...
FAST_FSS_DEVICE static std::uint8_t SBOX[256] = {
...
FAST_FSS_DEVICE static const std::uint32_t TE0[256] = {
...
FAST_FSS_DEVICE static const std::uint32_t TE1[256] = {
...
FAST_FSS_DEVICE static const std::uint32_t TE2[256] = {
...
FAST_FSS_DEVICE static const std::uint32_t TE3[256] = {
 ...
 */
FAST_FSS_DEVICE static inline void aes128_enc_key_init(
 ...
 */
FAST_FSS_DEVICE static inline void aes_enc_round(
    std::uint32_t       s[4],
                                                         ...
                                                         int round) noexcept
{
    ...
    tmp |= SBOX[(tmp1 >> 0) & 0xFF] << 8;
    tmp |= SBOX[(tmp1 >> 8) & 0xFF] << 16;
    tmp |= (SBOX[(tmp1 >> 16) & 0xFF] ^ Rcon[round - 1]) << 24;

    std::uint32_t w4 = w[0] ^ tmp;
    std::uint32_t w5 = w4 ^ w[1];
    std::uint32_t w6 = w5 ^ w[2];
    std::uint32_t w7 = w6 ^ w[3];
 ...
 */
FAST_FSS_DEVICE inline void aes128_compute_block(
...
template <int N>
FAST_FSS_DEVICE inline void aes128_compute_n_block(
    const std::uint32_t round_key[44],
    void               *ciphertext,
    const void         *plaintext) noexcept
{
    ...
    for (int i = 1; i < nr; i++)
    {
        for (int j = 0; j < N; j++)
        {
            aes_enc_round(state[j], round_key + 4 * i);
        }
    }
    for (int j = 0; j < N; j++)
    {
        aes_enc_last(state[j], round_key + 4 * nr);
    }
    //-----------Store as BigEndian--------------
    for (int j = 0; j < N; j++)
    {
...
template <int N>
FAST_FSS_DEVICE inline void aes128_enc_n_block(void       *ciphertext,
                                               ...
                                               const void *user_key) noexcept
{
    ...
    for (int i = 0; i < 4; i++)
    {
        w[i] = MEM_LOAD32BE((const char *)user_key + 4 * i);
    }
    std::uint32_t state[N][4];
    //----------------AddRoundKey----------------
    for (int j = 0; j < N; j++)
    {
        state[j][0] = MEM_LOAD32BE((const char *)plaintext + 16 * j);
        state[j][1] = MEM_LOAD32BE((const char *)plaintext + 16 * j + 4);
        state[j][2] = MEM_LOAD32BE((const char *)plaintext + 16 * j + 8);
        state[j][3] = MEM_LOAD32BE((const char *)plaintext + 16 * j + 12);
    ...
    for (int i = 1; i < 10; i++)
    {
        aes_key_schedule_next(w, i);
        for (int j = 0; j < N; j++)
        {
            aes_enc_round(state[j], w);
        }
    }
    aes_key_schedule_next(w, 10);
    for (int j = 0; j < N; j++)
    {
        aes_enc_last(state[j], w);
    }
    ...
    for (int j = 0; j < N; j++)
    {
        int offset = 16 * j;
        //-----------Store as BigEndian--------------
        MEM_STORE32BE((char *)ciphertext + offset, state[j][0]);
        MEM_STORE32BE((char *)ciphertext + offset + 4, state[j][1]);
        MEM_STORE32BE((char *)ciphertext + offset + 8, state[j][2]);
        MEM_STORE32BE((char *)ciphertext + offset + 12, state[j][3]);
    }
}
...
{
public:
    FAST_FSS_DEVICE static void aes128_enc1_block(
        void                      *ciphertext,
        const void                *plaintext,
        const void                *user_key,
    ...
    FAST_FSS_DEVICE static void aes128_enc2_block(
        void                      *ciphertext,
        const void                *plaintext,
        const void                *user_key,
        const AES128GlobalContext *ctx) noexcept
    ...
    FAST_FSS_DEVICE static void aes128_enc4_block(
        void                      *ciphertext,
        const void                *plaintext,
        const void                *user_key,
        const AES128GlobalContext *ctx) noexcept
