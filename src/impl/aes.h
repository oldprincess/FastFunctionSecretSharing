...
#define MEM_LOAD32BE(src)                                                                                        \
    (((std::uint32_t)(((std::uint8_t *)(src))[0]) << 24) | ((std::uint32_t)(((std::uint8_t *)(src))[1]) << 16) | \
     ((std::uint32_t)(((std::uint8_t *)(src))[2]) << 8) | ((std::uint32_t)(((std::uint8_t *)(src))[3]) << 0))
 ...
 */
FAST_FSS_DEVICE static inline void aes128_enc_key_init(std::uint32_t      round_key[44],
 ...
 */
FAST_FSS_DEVICE static inline void aes_enc_round(std::uint32_t s[4], const std::uint32_t rk[4]) noexcept
{
    std::uint32_t t[4];
 ...
 * @param rk    4-dword round key (128-bit)
 */
FAST_FSS_DEVICE static inline void aes_enc_last(std::uint32_t s[4], const std::uint32_t rk[4]) noexcept
{
    std::uint32_t t[4];
    //------------ShiftRow + SubByte-----------
    // t0
...
FAST_FSS_DEVICE static inline void aes_key_schedule_next(std::uint32_t w[4], int round) noexcept
{
    static const std::uint32_t Rcon[10] = {
        0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36,
    };
 ...
 */
FAST_FSS_DEVICE inline void aes128_compute_block(const std::uint32_t round_key[44],
                                                 void               *ciphertext,
...
template <int N>
FAST_FSS_DEVICE inline void aes128_compute_n_block(const std::uint32_t round_key[44],
                                                   void               *ciphertext,
                                                   const void         *plaintext) noexcept
{
    constexpr int nr = 10;
...
template <int N>
FAST_FSS_DEVICE inline void aes128_enc_n_block(void *ciphertext, const void *plaintext, const void *user_key) noexcept
{
    std::uint32_t w[4];
    //--------------Load as BigEndian---------------
...
{
public:
    FAST_FSS_DEVICE static void aes128_enc1_block(void       *ciphertext,
...
public:
    FAST_FSS_DEVICE void set_enc_key(const void *user_key) noexcept
    {
        internal::aes128_enc_key_init(rk_, (const std::uint8_t *)user_key);
    }

    FAST_FSS_DEVICE void enc_block(void *out, const void *in) noexcept
    {
        internal::aes128_compute_block(rk_, out, in);
    }

    FAST_FSS_DEVICE void enc4_block(void *out, const void *in) noexcept
    {
        internal::aes128_compute_n_block<4>(rk_, out, in);
    }

    template <int N>
    FAST_FSS_DEVICE void enc_n_block(void *out, const void *in) noexcept
    {
        internal::aes128_compute_n_block<N>(rk_, out, in);
    }

    FAST_FSS_DEVICE void enc_blocks(void *out, const void *in, std::size_t block_num) noexcept
    {
