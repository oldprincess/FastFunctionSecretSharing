...
template <typename GroupElement>
__device__ inline void grottoKeySetPtr(GrottoKey<GroupElement>& grottoKey,
                                       const void*              keyData,
...
template <typename GroupElement>
__device__ inline void grottoCacheSetPtr( //
...
template <typename GroupElement>
__device__ inline void grottoKeyGen(GrottoKey<GroupElement>& key,
                                    GroupElement             alpha,
                                    const void*              seed0,
                                    const void*              seed1,
...
template <typename GroupElement>
__global__ void grottoKeyGenKernel(void*       deviceKey,
                                   ...
                                   std::size_t elementNum)
{
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        ...
        grottoKeySetPtr(grottoKey, deviceKey, bitWidthIn, i, elementNum);
        std::size_t offsetSeed0 = i * 16;
        std::size_t offsetSeed1 = i * 16;
        grottoKeyGen(grottoKey, deviceAlphaPtr[i], deviceSeedPtr0 + offsetSeed0,
                     deviceSeedPtr1 + offsetSeed1, bitWidthIn);
    }
}

template <typename GroupElement>
__device__ inline GroupElement grottoEvalEq( //
...
template <typename GroupElement>
__global__ void grottoEvalEqKernel(void*       deviceSharedBooleanOut,
                                   ...
                                   std::size_t elementNum)
{
    ...
    GroupElement* sharedBooleanOutPtr = (GroupElement*)deviceSharedBooleanOut;
    const GroupElement* maskedXPtr    = (const GroupElement*)deviceMaskedX;
    const std::uint8_t* seedPtr       = (const std::uint8_t*)deviceSeed;
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        ...
        sharedBooleanOutPtr[i] =
            grottoEvalEq(grottoKey, maskedXPtr[i], seedPtr + offsetSeed,
                         partyId, bitWidthIn);
    }
}

template <typename GroupElement>
__device__ inline GroupElement grottoEval( //
    const GrottoKey<GroupElement>& key,
    GroupElement                   maskedX,
    const void*                    seed,
    int                            partyId,
...
template <typename GroupElement>
__global__ void grottoEvalKernel(void*       deviceSharedBooleanOut,
                                 const void* deviceKey,
                                 const void* deviceMaskedX,
                                 const void* deviceSeed,
                                 int         partyId,
                                 ...
                                 std::size_t elementNum)
{
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        GrottoKey<GroupElement>    grottoKey;
        GrottoCache<GroupElement>  cache;
        GrottoCache<GroupElement>* cachePtr = nullptr;
        ...
        sharedBooleanOutPtr[i] =
            grottoEval(grottoKey, maskedXPtr[i], seedPtr + offsetSeed, partyId,
                       bitWidthIn, equalBound, cachePtr);
    }
}

template <typename GroupElement>
__device__ inline void grottoMICEval(         //
...
template <typename GroupElement>
__global__ void grottoMICEvalKernel(void*       deviceSharedBooleanOut,
                                    const void* deviceMaskedX,
                                    const void* deviceKey,
                                    const void* deviceSeed,
                                    int         partyId,
                                    const void* deviceLeftBoundary,
                                    const void* deviceRightBoundary,
                                    ...
                                    std::size_t elementNum)
{
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        GrottoKey<GroupElement>    grottoKey;
        GrottoCache<GroupElement>  cache;
        GrottoCache<GroupElement>* cachePtr;
        ...
        grottoMICEval(sharedBooleanOutPtr + offsetSharedOut, maskedXPtr[i],
                      grottoKey, seedPtr + offsetSeed, partyId,
                      (const GroupElement*)deviceLeftBoundary,
                      (const GroupElement*)deviceRightBoundary, intervalNum,
                      bitWidthIn, cachePtr);
    }
}

template <typename GroupElement>
__device__ inline void grottoIntervalLutEval( //
...
template <typename GroupElement>
__global__ void grottoIntervalLutEvalKernel(void*       deviceSharedOutE,
                                            void*       deviceSharedOutT,
                                            const void* deviceMaskedX,
                                            const void* deviceKey,
                                            const void* deviceSeed,
                                            int         partyId,
                                            ...
                                            std::size_t elementNum)
{
    ...
    GroupElement*       sharedOutEPtr = (GroupElement*)deviceSharedOutE;
    GroupElement*       sharedOutTPtr = (GroupElement*)deviceSharedOutT;
    const GroupElement* maskedXPtr    = (const GroupElement*)deviceMaskedX;
    const std::uint8_t* seedPtr       = (const std::uint8_t*)deviceSeed;
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        ...
        grottoKeySetPtr(grottoKey, deviceKey, bitWidthIn, i, elementNum);
        std::size_t offsetSharedOutE = i;
        std::size_t offsetSharedOutT = i;
        std::size_t offsetSeed       = i * 16;
        if (deviceCache != nullptr)
        {
            grottoCacheSetPtr(cache, deviceCache, bitWidthIn, i % stride,
        ...
        grottoIntervalLutEval(sharedOutEPtr + offsetSharedOutE,         //
                              sharedOutTPtr + offsetSharedOutT,         //
                              maskedXPtr[i],                            //
                              grottoKey,                                //
                              seedPtr + offsetSeed,                     //
                              partyId,                                  //
                              (const GroupElement*)deviceLeftBoundary,  //
                              (const GroupElement*)deviceRightBoundary, //
                              (const GroupElement*)deviceLoopUpTable,   //
                              intervalNum,                              //
                              bitWidthIn,                               //
                              cachePtr                                  //
        );
