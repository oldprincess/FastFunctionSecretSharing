#ifndef SRC_FAST_FSS_CUDA_AES_CUH
#define SRC_FAST_FSS_CUDA_AES_CUH
...
static __device__ const std::uint8_t SBOX[256] = {
...
static __device__ const std::uint8_t INV_SBOX[256] = {
...
static __device__ const std::uint32_t TE0[256] = {
...
static __device__ const std::uint32_t TE1[256] = {
...
static __device__ const std::uint32_t TE2[256] = {
...
static __device__ const std::uint32_t TE3[256] = {
 ...
 */
inline __device__ void aes128_enc_key_init(
 ...
 */
inline __device__ void aes_enc_round(std::uint32_t       s[4],
 ...
 * @param rk    4-dword round key (128-bit)
 */
inline __device__ void aes_enc_last(std::uint32_t       s[4],
                                    const std::uint32_t rk[4]) noexcept
{
    std::uint32_t t[4];
    //------------ShiftRow + SubByte-----------
...
inline __device__ void aes_key_schedule_next(std::uint32_t w[4],
                                             int           round) noexcept
{
    static const std::uint32_t Rcon[10] = {
        0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36,
 ...
 */
inline __device__ void aes128_compute_block(const std::uint32_t round_key[44],
                                            void               *ciphertext,
...
template <int N>
inline __device__ void aes128_compute_n_block(const std::uint32_t round_key[44],
                                              void               *ciphertext,
                                              const void *plaintext) noexcept
{
    int           nr = 10;
...
template <int N>
inline __device__ void aes128_enc_n_block(void       *ciphertext,
...
{
public:
    __device__ static void aes128_enc1_block(void       *ciphertext,
                                             const void *plaintext,
                                             const void *user_key) noexcept
    {
    ...
    __device__ static void aes128_enc2_block(void       *ciphertext,
                                             const void *plaintext,
                                             const void *user_key) noexcept
    {
    ...
    __device__ static void aes128_enc4_block(void       *ciphertext,
                                             const void *plaintext,
                                             const void *user_key) noexcept
    {
...
public:
    __device__ void set_enc_key(const void *user_key) noexcept
    {
        internal::aes128_enc_key_init(rk_, (const std::uint8_t *)user_key);
    }

    __device__ void enc_block(void *out, const void *in) noexcept
    {
        internal::aes128_compute_block(rk_, out, in);
    }

    __device__ void enc4_block(void *out, const void *in) noexcept
    {
        internal::aes128_compute_n_block<4>(rk_, out, in);
    }

    template <int N>
    __device__ void enc_n_block(void *out, const void *in) noexcept
    {
