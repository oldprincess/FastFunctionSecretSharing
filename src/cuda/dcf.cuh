#pragma once
#ifndef SRC_FAST_FSS_CUDA_DCF_CUH
#define SRC_FAST_FSS_CUDA_DCF_CUH
...
template <typename GroupElement>
static __device__ GroupElement convert(const std::uint64_t s[2],
...
template <typename GroupElement>
__device__ inline void dcfKeySetPtr(DcfKey<GroupElement>& dcfKey,
                                    const void*           keyData,
...
template <typename GroupElement>
inline __device__ void dcfKeyGen(DcfKey<GroupElement>& key,
                                 GroupElement          alpha,
                                 GroupElement          beta,
                                 const void*           seed0,
                                 const void*           seed1,
                                 ...
                                 std::size_t           bitWidthOut) noexcept
{
    ...
    for (int i = 0; i < bitWidthIn; i++)
    {
        ...
        vAlpha += -convert<GroupElement>(vKeep1, bitWidthOut) +
...
template <typename GroupElement>
__global__ void dcfKeyGenKernel(void*       deviceKey,
                                const void* deviceAlpha,
                                const void* deviceBeta,
                                ...
                                std::size_t elementNum)
{
    ...
    const GroupElement* deviceBetaPtr  = (const GroupElement*)deviceBeta;
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        ...
        GroupElement curBeta = (deviceBeta == nullptr) ? 1 : deviceBetaPtr[i];
        dcfKeyGen(dcfKey, deviceAlphaPtr[i], curBeta,
...
template <typename GroupElement>
inline __device__ GroupElement dcfEval( //
...
template <typename GroupElement>
__global__ void dcfEvalKernel(void*       deviceSharedOut,
                              ...
                              std::size_t elementNum)
{
    ...
    for (std::size_t i = idx; i < elementNum; i += stride)
    {
        DcfKey<GroupElement> dcfKey;
        dcfKeySetPtr(dcfKey, deviceKey, bitWidthIn, bitWidthOut, i, elementNum);
        deviceSharedOutPtr[i] =
            dcfEval(dcfKey, deviceMaskedXPtr[i], deviceSeedPtr + i * 16,
